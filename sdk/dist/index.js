...
__export(index_exports, {
  ...
  HeliusEnhanced: () => HeliusEnhanced,
  LocalRelayer: () => LocalRelayer,
  NaturalLanguageAshborn: () => NaturalLanguageAshborn,
  NaturalLanguageParser: () => NaturalLanguageParser,
  PROGRAM_ID: () => PROGRAM_ID,
  PrivacyCash: () => PrivacyCash,
  PrivateNFTManager: () => PrivateNFTManager,
  ProofType: () => ProofType,
  RELAYER_ENDPOINTS: () => RELAYER_ENDPOINTS,
  ...
  createHeliusEnhanced: () => createHeliusEnhanced,
  createNFTPrivacy: () => createNFTPrivacy,
  createRangeCompliance: () => createRangeCompliance,
  createRelayer: () => createRelayer,
  createTransferWithDecoys: () => createTransferWithDecoys,
  createTreeIndexer: () => createTreeIndexer,
  decryptAmount: () => decryptAmount,
  decryptNote: () => decryptNote,
  deriveEncryptionKey: () => deriveEncryptionKey,
  deriveStealthAddress: () => deriveStealthAddress,
  encryptAmount: () => encryptAmount2,
  encryptNote: () => encryptNote,
  findRealOutput: () => findRealOutput,
  generateDecoys: () => generateDecoys,
  generateNullifier: () => generateNullifier2,
  poseidonHash: () => poseidonHash,
  prepareForRelay: () => prepareForRelay,
...
var snarkjs2 = __toESM(require("snarkjs"));
...
var ShadowWire = class {
  ...
  async generateStealthAddress() {
    const ephemeral = import_web3.Keypair.generate();
    ...
    const stealthKeypair = import_web3.Keypair.generate();
    return {
...
var snarkjs3 = __toESM(require("snarkjs"));
var PrivacyCash = class {
  ...
  getSupportedTokens() {
    return [
      ...
      // SOL
      new import_web32.PublicKey("EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v"),
      // USDC
      new import_web32.PublicKey("Es9vMFrzaCERmJfrF4H2FYD4KCoNkY11McCe8BenwNYB")
      // USDT
    ];
  }
};
...
var crypto2 = globalThis.crypto ?? require("crypto").webcrypto;
...
var RangeCompliance = class _RangeCompliance {
  ...
  constructor(connection, wallet, programId = new import_web33.PublicKey("ASHBrnShdwMnrch1111111111111111111111111")) {
  ...
  async createViewKeyAuthorization(viewer, scope, expiresAt) {
    ...
    if (this.wallet.signMessage) {
      ...
      signature = (0, import_sha256.sha256)(message);
    }
    return {
      viewer,
      scope,
  ...
  signSchnorr(privateKey, message) {
    ...
    const R = import_secp256k1.secp256k1.ProjectivePoint.BASE.multiply(bytesToBigint(k));
    const r = bigintToBytes2(R.x, 32);
    const pubKey = import_secp256k1.secp256k1.getPublicKey(privKey, true);
    const e = (0, import_sha256.sha256)(new Uint8Array([...r, ...pubKey, ...message]));
    const eBigint = bytesToBigint(e);
    const xBigint = bytesToBigint(privKey);
    const kBigint = bytesToBigint(k);
    const sBigint = (kBigint + eBigint * xBigint) % import_secp256k1.secp256k1.CURVE.n;
  ...
  verifySchnorr(r, s, message, publicKeyBytes) {
    try {
      const e = (0, import_sha256.sha256)(new Uint8Array([...r, ...publicKeyBytes, ...message]));
      const eBigint = bytesToBigint(e);
      const sBigint = bytesToBigint(s);
      const sG = import_secp256k1.secp256k1.ProjectivePoint.BASE.multiply(sBigint);
      const P = import_secp256k1.secp256k1.ProjectivePoint.fromHex(publicKeyBytes);
      const R = import_secp256k1.secp256k1.ProjectivePoint.fromAffine({
        x: bytesToBigint(r),
        y: 0n
...
var PROGRAM_ID = new import_web34.PublicKey(
...
var Ashborn = class {
   ...
   */
  getVaultAddress() {
    const [vaultPda] = import_web35.PublicKey.findProgramAddressSync(
  ...
  getNoteAddress(vaultAddress, index) {
    const [notePda] = import_web35.PublicKey.findProgramAddressSync(
      [
        Buffer.from(SEEDS.SHIELDED_NOTE),
        vaultAddress.toBuffer(),
        new import_anchor.BN(index).toArrayLike(Buffer, "le", 4)
      ],
      this.programId
    );
    return notePda;
  ...
  async shield(params) {
    ...
    const commitment = await this.privacyCash.createShieldCommitment(
      amount,
      blindingFactor ?? import_web35.Keypair.generate().secretKey.slice(0, 32)
    );
    const vaultAddress = this.getVaultAddress();
  ...
  async getVault() {
    try {
      ...
      return {
        owner: new import_web35.PublicKey(accountInfo.data.slice(8, 40)),
        bump: accountInfo.data[40],
        shadowBalance: new import_anchor.BN(accountInfo.data.slice(41, 49), "le"),
        noteCount: accountInfo.data.readUInt32LE(49),
        viewKeyHash: accountInfo.data.slice(53, 85),
        createdAt: new import_anchor.BN(accountInfo.data.slice(85, 93), "le"),
        lastActivity: new import_anchor.BN(accountInfo.data.slice(93, 101), "le")
      };
    } catch {
      return null;
    }
  ...
  async getNote(noteAddress) {
    try {
      ...
      if (!accountInfo) return null;
      return {
        vault: new import_web35.PublicKey(accountInfo.data.slice(8, 40)),
        ...
        createdAt: new import_anchor.BN(accountInfo.data.slice(125, 133), "le"),
  ...
  async createShieldInstruction(vaultAddress, noteAddress, amount, commitment, _mint) {
    ...
    const data = Buffer.concat([
      discriminator,
      new import_anchor.BN(amount.toString()).toArrayLike(Buffer, "le", 8),
  ...
  async createTransferInstruction(vaultAddress, sourceNoteAddress, nullifier, senderCommitment, recipientCommitment, proof) {
    ...
    const data = Buffer.concat([
      ...
      new import_anchor.BN(proof.length).toArrayLike(Buffer, "le", 4),
      Buffer.from(proof)
    ]);
    return new import_web35.TransactionInstruction({
      keys: [
  ...
  async createRevealInstruction(vaultAddress, proofType, rangeMin, rangeMax, proofData) {
    ...
    const data = Buffer.concat([
      ...
      new import_anchor.BN(rangeMin.toString()).toArrayLike(Buffer, "le", 8),
      new import_anchor.BN(rangeMax.toString()).toArrayLike(Buffer, "le", 8),
      new import_anchor.BN(proofData.length).toArrayLike(Buffer, "le", 4),
...
var HeliusEnhanced = class {
  ...
  constructor(config) {
    ...
    this.connection = new import_web36.Connection(`${this.baseUrl}?api-key=${this.apiKey}`);
  }
  // ============================================================
  ...
  createComputeBudgetInstructions(priorityFee) {
    return [
      ...
      // Safe default
      import_web36.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: priorityFee })
    ];
  }
  /**
  ...
  async sendSmartTransaction(transaction, signers, options) {
    const feeEstimate = await this.getPriorityFeeEstimate(
      transaction instanceof import_web36.Transaction ? transaction.instructions.flatMap((ix) => ix.keys.map((k) => k.pubkey)) : []
    );
    ...
    if (transaction instanceof import_web36.Transaction) {
      const computeIx = import_web36.ComputeBudgetProgram.setComputeUnitLimit({
        units: 4e5
      });
      const priorityIx = import_web36.ComputeBudgetProgram.setComputeUnitPrice({
...
async function prepareForRelay(transaction, userSigners, connection) {
  ...
  transaction.feePayer = new import_web37.PublicKey(
...
var LocalRelayer = class {
  ...
  async relay(serializedTx, userSignatures) {
    ...
    const tx = import_web37.Transaction.from(txBuffer);
    ...
    for (const sig of userSignatures) {
      ...
      tx.addSignature(new import_web37.PublicKey(sig.pubkey), sigBuffer);
    }
...
var PrivateNFTManager = class {
  ...
  async unshieldNFT(privateNFT, viewKey, nullifierSecret) {
    ...
    const owner = new import_web38.PublicKey(ownerBytes);
